...
40. **Probable wins nudge + auto-detect (daily):** The `probable-wins-nudge` cron runs daily at 10am UTC and prompts contractors to mark leads won or lost when an appointment is 7+ days old with no resolution. Messages include outcome reference codes (e.g., &ldquo;Ref 4A&rdquo;). Contractors reply WON 4A or LOST 4A to their business number. The WINS command lists recent pending leads with ref codes. WON triggers review request + logs revenue. LOST cancels sequences. PAUSE/RESUME commands give contractors control over automation. If a client reports issues, verify: (a) phone configured, (b) status not paused, (c) 7-day cooldown not tripped.
40a. **Proactive quote prompt (daily):** The `proactive-quote-prompt` cron runs daily at 10am UTC. When a lead has been in `new` or `contacted` for 3+ days with no EST trigger, the contractor gets: &ldquo;[Name] has been waiting 3 days. Reply EST [Name] or PASS.&rdquo; Fires once per lead (audit_log tracked). This catches missed estimates that the 24h fallback nudge didn&apos;t catch.
40b. **Booking confirmation mode:** For non-Google-Calendar clients with `bookingConfirmationRequired = true`, AI booking requests go through a contractor approval step before confirming with the homeowner. The contractor gets an SMS with the booking details and replies YES or suggests a new time. 2-hour reminder + 4-hour operator escalation if no response. Set this toggle during onboarding for any contractor who answers &ldquo;no&rdquo; to &ldquo;Do you use Google Calendar daily?&rdquo;

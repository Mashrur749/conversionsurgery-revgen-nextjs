...
6. Day-One activation workflow (`MS-09`) adds:
...
8. Report delivery observability (`MS-11` Milestones A-C) adds lifecycle/retry services, cron retry endpoint, and analytics-scoped admin delivery ops APIs (`/api/admin/reports/deliveries`, `/api/admin/reports/deliveries/[deliveryId]/retry`); no new public endpoints or elevated permissions were introduced.

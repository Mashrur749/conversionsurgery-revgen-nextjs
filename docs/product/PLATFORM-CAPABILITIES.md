...
- **Hourly AI quality alert:** `ai-quality-alert` cron (hourly) checks two thresholds: (1) average decision confidence drops below 40 across 5+ decisions in the last hour, (2) output guard blocked 3+ messages in the last hour. Either condition triggers an operator SMS alert with details. Deduplicates via audit_log (max 1 alert per hour). Catches real-time quality degradation (e.g., API returning low-quality responses overnight) that the weekly health check would miss.

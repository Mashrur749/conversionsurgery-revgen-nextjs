...
export async function POST(request: NextRequest) {
  try {
    ...
    void logInternalError({
      source: '[Voice] Ring result webhook',
      error,
      context: { route: '/api/webhooks/twilio/ring-result' },
    });
    logSanitizedConsoleError('[Voice] Ring result webhook failed', error, {
      route: '/api/webhooks/twilio/ring-result',
    });
    const twiml = new VoiceResponse();
    twiml.hangup();
    return new NextResponse(twiml.toString(), {

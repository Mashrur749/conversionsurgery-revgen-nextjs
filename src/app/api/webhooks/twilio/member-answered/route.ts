  ...
  try {
    ...
    if (client?.twilioNumber) {
      ...
      for (const otherMember of otherMembers) {
        ...
        await sendSMS(
          otherMember.phone,
          `Answered: ${member?.name || 'Team member'} is on the call with ${leadDisplay}.`,
          client.twilioNumber
        );
      }
    }
    ...
    void logInternalError({
      source: '[Voice] Member answered webhook',
      error,
      context: { route: '/api/webhooks/twilio/member-answered' },
    });
    logSanitizedConsoleError('[Voice] Member answered webhook failed', error, {
      route: '/api/webhooks/twilio/member-answered',
    });
    return new NextResponse('OK');
  }
}

...
1. Read `.Codex/skills/ux-standards/SKILL.md` first — it defines patterns, anti-patterns, and brand conventions
...
- `pnpm run quality:code-review` — automated code review via `Codex -p` against diff from main (fresh context window)
...
| Config files, project-level docs                       | Root (`./`)            | Only: README.md, AGENTS.md, BUSINESS-CASE.md, DEPLOYMENT.md |
...
4. **Cleanup is automatic** — the Stop hook runs `.Codex/scripts/cleanup.sh` on session end. It purges `.scratch/`, stale artifacts, `.DS_Store` files, and warns about untracked root files.
5. **Manual cleanup**: `bash .Codex/scripts/cleanup.sh`
...
**Agent prompt template (keep lean — don't repeat AGENTS.md rules):**
```
Fix [ITEM-IDs]: [brief description]

Read `.Codex/skills/ux-standards/SKILL.md` for patterns and rules.
...
- Schema changes: read `.Codex/skills/create-migration/SKILL.md` first
- Neon queries: read `.Codex/skills/neon-postgres/` for patterns
- Security review: run on any slice touching API routes, auth, or user input

## Agent Orchestration

These scripts enforce safe, unattended agent execution. Agents must use them instead of raw git commands.
...
If the script exits non-zero, stop and resolve the issue before coding.

### Checkpoint Commits

Run `pnpm run agent:ship "[area] verb noun — context"` every 3–5 files (or one coherent unit of work, whichever is smaller).

This script:
- Runs `pnpm run typecheck`
- Runs `pnpm run quality:logging-guard`
- Stages all changes and commits only if both gates pass
- Generates a fallback message from changed files if none is provided

**Agents never use raw `git commit` or `git push`.** Always use `agent:ship`.
...
`pnpm run agent:ship --force "message"` skips gates. Use this only for genuine emergencies (e.g., fixing a broken build that blocks a production deploy).

If `--force` is used, the agent **must** log a memory entry explaining why before stopping. The override prints a loud warning as a reminder.
...
11. [PROCESS] Every UI/UX change must follow the process in `.Codex/skills/ux-standards/SKILL.md` — read skill first, reuse established patterns, update UX audit doc + all matching docs from Change-to-Doc table.
...
<!-- Memory context removed: this block was maintained by Claude's context-mode plugin.
     Without the plugin, it becomes stale and violates the "no volatile memory exports"
     rule in this file. For cross-session context, see .planning/PROJECT.md and
     .claude/progress.md (if using GSD workflows). -->
...
Consolidated seed data required for a fresh ConversionSurgery deployment. Run this AFTER all migrations complete.
    ...
    steps: [
      {
        stepNumber: 1,
        name: "Day Before",
        delayMinutes: -1440, // Negative = before appointment
        messageTemplate:
      ...
      },
      {
        stepNumber: 2,
        name: "Morning Of",
        delayMinutes: -180, // 3 hours before
        messageTemplate:
    ...
    steps: [
      {
        ...
        delayMinutes: 1440, // Day after completion
        messageTemplate:
      ...
      },
      {
        ...
        delayMinutes: 10080, // 7 days later
        messageTemplate:
...
**ADD to `src/lib/db/schema.ts`** (if not already present):
...
Before running seed, create these products in Stripe Dashboard:

### Products

| Product      | Stripe ID         | Description        |
| ------------ | ----------------- | ------------------ |
| Starter      | `prod_starter`    | Entry-level plan   |
| Professional | `prod_pro`        | Full-featured plan |
| Enterprise   | `prod_enterprise` | High-volume plan   |
...
| ------------ | --------- | ---------- | ------------- |
| Starter      | $497/mo   | $4,970/yr  | 2 months free |
| Professional | $997/mo   | $9,970/yr  | 2 months free |
| Enterprise   | $1,997/mo | $19,970/yr | 2 months free |

### Metered Billing (Overages)

Create metered prices for overages:

- Additional leads: $0.50 each
- Additional messages: $0.03 each
- Additional team members: $20.00/month each
- Additional phone numbers: $15.00/month each
- Voice AI minutes: $0.15/minute
...
- Plans: Updates if exists, creates if not
- Templates: Updates if exists, recreates steps
- Admin: Skips if exists
- Settings: Skips if exists
...
- [ ] Verify Stripe product/price IDs match your Stripe Dashboard
- [ ] Test subscription flow in Stripe test mode
- [ ] Verify flow templates appear in admin UI
...
- Phase 01: Database setup (migrations)
- Phase 14a: Flow schema (flowTemplates, flowTemplateSteps tables)
- Phase 41: Billing schema (subscriptionPlans table)
- Phase 07a: Admin schema (adminUsers table)
